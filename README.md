...
- Install `Lightdm` (RECOMMEND if do not have any login manager yet) for login manager also custom script for monitor setup
    ...
    ```
    For someone who's using multiple-monitor setup just use ***[autorandr](https://github.com/phillipberndt/autorandr)***

    (**PERSONAL SETUP & UNSTABLE**) Below step ONLY for my own specifics setup add **custom script** to lightdm
